...
class Upload_Photos_Controller extends Admin_Pages_Controller {
    ...
    $this->template->content = new View("admin/upload_photos");
  ...
  public function upload() {
    ...
    if ($this->input->post("deactivate_all") == true && $this->input->post("activate_all") == true) {
      Event::run("system.404");
    }

    if ($this->input->post("deactivate_all") == true || $this->input->post("activate_all") == true) {
      $this->deactivate_or_activate_all();
    }
    
    // uses Kohana upload helper
    $_FILES = Validation::factory($_FILES)
      ->add_rules('file', 'upload::valid', 'upload::type[gif,jpg,png]', 'upload::size[10M]');
    if ($_FILES->validate()) {
  ...
  public function view($image_id) {
    ...
    if ($get_image == null) Event::run("system.404");
    $this->template->content = new View("admin/image_options");
  ...
  public function update($img_id) {
    ...
    if ($this->input->post("check_profile") == true) {
      $get_user = Doctrine::getTable("Tblusers")->find($this->session->get("author"));
    ...
    url::redirect("admin/upload_photos");
  }
