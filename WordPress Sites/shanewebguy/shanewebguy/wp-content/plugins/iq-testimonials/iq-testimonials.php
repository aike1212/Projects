...
define('IQ_TESTIMONIALS_DB_VERSION', '2.1.0');
...
function iq_testimonials_install(){
	...
	update_option('iq-testimonials-db-version', IQ_TESTIMONIALS_DB_VERSION);
		...
		" (id, name, value) " .
		"VALUES ('1','admin_email','$admin_email'), ('2','date','1'), ('3','max_widget_testimonials','1'), ('4','max_page_testimonials','15'), ('5','max_image_width','125'), ('6','max_image_height','125'), ('7', 'randomize', '1')";
		...
		$update_option = "UPDATE $iq_testimonials_settings SET name='randomize' WHERE id=7";
		$results = $wpdb->query($update_option);
		$update_option = "UPDATE $iq_testimonials_settings SET value='0' WHERE name='randomize'";
		$results = $wpdb->query($update_option);
		
		update_option('iq-testimonials-db-version', IQ_TESTIMONIALS_DB_VERSION);
		}

//		$update_option = "UPDATE $iq_testimonials_settings SET value='$db_version_new' WHERE name='db_version'";
...
function iq_testimonials($id='', $is_shortcode){
	...
	{
//		$query = "SELECT * FROM $testimonials WHERE status!='1' AND status!='2' ORDER BY featured=1 DESC LIMIT $max_widget_testimonials";
		...
		while ($data = mysql_fetch_array($results)){
			...
			$website = str_replace('http://','',$data['website']);
				if (!empty($website)){					
					$websitepre = '<a href="http://'.$website.'" target="_blank">';
					$websitesuf = '</a>';
				}
			...
			// Final HTML Output
			$theTestimonial .= $featuredpre . $imagelink .' '. $quote . '<br /><span> <strong>~' .$name . '</strong><em>, ' . $location . '</em><br />' . $websitepre . $website . $websitesuf . "</span></div>";
			$i++;
		}
