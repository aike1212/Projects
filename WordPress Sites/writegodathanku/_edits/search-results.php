<?php
	...
	if(!isset($_GET['search']) || $_GET['search'] == 'Search') {
		...
		*/
		...
		$query = "
			SELECT firstname.meta_value AS first_name, lastname.meta_value AS last_name, users.user_nicename, users.profile_picture_url
			FROM wp_users AS users
			LEFT JOIN wp_usermeta AS firstname ON users.ID = firstname.user_id
			LEFT JOIN wp_usermeta AS lastname ON firstname.user_id = lastname.user_id
		";
	...
	<?php if ($result != null) { ?>
		...
		<?php foreach($result as $user) { ?>
			<p class="left result-holder">
				...
				<span class="block"><?php echo "<a href='".get_site_url()."/author/".$user->user_nicename."'>".$user->user_nicename."</a>"; ?></span>
				<span class="block"><?php echo $user->first_name . " " . $user->last_name ?> </span>
			</p>
		<?php } ?>
